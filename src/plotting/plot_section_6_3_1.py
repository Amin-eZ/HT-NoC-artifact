...
def load_rows(csv_path):
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            parsed = dict(row)
            for key in ["weight", "activation", "compute", "sum_collection", "reconfiguration", "total", "speedup_vs_baseline"]:
...
def plot(rows, out_path):
    ...
    for model, noc in experiments:
        ...
        baseline_row = next(
            r for r in rows if r["model"] == model and r["noc"] == noc and r["architecture"] == "baseline"
        )
        ht_row = next(
            r for r in rows if r["model"] == model and r["noc"] == noc and r["architecture"] == "htnoc"
        )
    ...
    ax.bar(baseline_x, baseline_activation, width, bottom=baseline_weight, label="Activation (Baseline)")
    ax.bar(
        baseline_x,
        baseline_compute,
        width,
    ...
    ax.bar(ht_x, ht_activation, width, bottom=ht_weight, label="Activation (HT-NoC)")
    ax.bar(
        ht_x,
        ht_compute,
        width,
    ...
    ax.set_title("Section 6.3.1 – GPT-2 FFN execution time breakdown")
...
def main():
    ...
    csv_path = root / "outputs" / "section_6_3_1"/ "tables" / "section_6_3_1_breakdown.csv"
    fig_dir = root / "outputs" / "section_6_3_1"/ "figures"
    fig_dir.mkdir(parents=True, exist_ok=True)
    out_path = fig_dir / "section_6_3_1_breakdown.png"
